...
# IdentityMap / Map: type-agnostic key->value table using open addressing (linear
...
# _im_kvhash must be replaced; the rest still works.
#
# Slots: 0=empty, 1=live, 2=tombstone. A single _im_kvwalk traverses a cluster
...
# updated IdentityMap; length counts live entries and used_count counts live +
...
public def _im_next_pow2(n size_t) -> size_t:
    size_t p = 8
    while p < n:
        p = p * (size_t)2
    return p

public def create_identity_map(cap size_t) -> IdentityMap:
    size_t c = _im_next_pow2(cap)
    IdentityMap m
    m.capacity = c
...
public def _im_kveq(a void*, b void*) -> int:
    int eq = 0
    if a == b:
        eq = 1
    return eq
...
public def _im_kvhash(key void*) -> size_t:
    size_t a = (size_t)key
    a = a ^ (a >> 33)
...
public def _im_kvwalk(m IdentityMap, start size_t, key void*) -> size_t:
    ...
    while stop == 0:
        ...
        if st == 0:
            return cap
        if st == 1 and _im_kveq(m.keys[idx], key) == 1:
            return idx
        idx = idx + (size_t)1
        if idx >= cap:
            idx = (size_t)0
...
public def _im_kvfree(m IdentityMap, start size_t) -> size_t:
    size_t cap = m.capacity
    size_t idx = start
    int stop = 0
    while stop == 0:
...
public def _im_kv_rehash(m IdentityMap, new_cap size_t, key void*, value void*, do_set int) -> IdentityMap:
    size_t c = _im_next_pow2(new_cap)
    IdentityMap nm
    nm.capacity = c
    ...
    while i < (int)m.capacity:
        if m.used[i] == 1:
            size_t start = _im_kvhash(m.keys[i]) & (nm.capacity - (size_t)1)
            size_t slot = _im_kvfree(nm, start)
        ...
        i = i + 1
    if do_set == 1:
        size_t start = _im_kvhash(key) & (nm.capacity - (size_t)1)
        size_t slot = _im_kvfree(nm, start)
        nm.keys[slot] = key
...
# Insert or update key->value; returns the updated IdentityMap.
public def identity_put(m IdentityMap, key void*, value void*) -> IdentityMap:
    size_t cap = m.capacity
    size_t ln = m.length
    size_t uc = m.used_count
    size_t start = _im_kvhash(key) & (cap - (size_t)1)
    size_t hit = _im_kvwalk(m, start, key)
    if hit != cap:
        m.values[hit] = value
        return m
    ...
    if grown >= loadthr or (uc + (size_t)1) * (size_t)10 >= upthr:
        return _im_kv_rehash(m, cap * (size_t)2, key, value, 1)
    size_t slot = _im_kvfree(m, start)
    m.keys[slot] = key
    m.values[slot] = value
    m.used[slot] = 1
...
public def identity_get(m IdentityMap, key void*) -> void*:
    size_t cap = m.capacity
    size_t start = _im_kvhash(key) & (cap - (size_t)1)
    size_t hit = _im_kvwalk(m, start, key)
    if hit != cap:
...
public def identity_contains(m IdentityMap, key void*) -> int:
    size_t cap = m.capacity
    size_t start = _im_kvhash(key) & (cap - (size_t)1)
    size_t hit = _im_kvwalk(m, start, key)
    int found = 0
...
# Remove a key (marks a tombstone); returns the IdentityMap. The slot stays
# occupied (as a tombstone) so in-progress lookups never truncate a cluster.
public def identity_erase(m IdentityMap, key void*) -> IdentityMap:
    size_t cap = m.capacity
    size_t ln = m.length
    size_t start = _im_kvhash(key) & (cap - (size_t)1)
    size_t hit = _im_kvwalk(m, start, key)
...
public def identity_size(m IdentityMap) -> size_t:
    return m.length

public def identity_is_empty(m IdentityMap) -> int:
    int e = 0
