...
extern void cpyte_array_register(void *arr, int64_t n);
...
void* deque_to_list(void *deque_ptr) {
    ...
    if (n == 0) {
        deque_dynvalue_t *empty = (deque_dynvalue_t *)malloc(sizeof(deque_dynvalue_t));
        if (!empty) return NULL;
        cpyte_array_register(empty, 0);
        return (void *)empty;
    }

    void **src = (void **)dq->data;
    deque_dynvalue_t *arr = (deque_dynvalue_t *)malloc(sizeof(deque_dynvalue_t) * n);
    if (!arr) return NULL;
    ...
    cpyte_array_register(arr, (int64_t)n);
    return (void *)arr;
}
"""
