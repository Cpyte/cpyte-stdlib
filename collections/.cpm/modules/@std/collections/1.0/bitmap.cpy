...
# (set/clear/flip/count over [lo, hi)). Masks are built at runtime from
# shifts; range ops touch only the boundary words plus a stride-1 interior
# scan, so the LLVM JIT can vectorize the interior loop.
