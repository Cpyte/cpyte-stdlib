...
# Cache-friendly flat pools: keys, values, prev, next are separate dense arrays.
...
public def lru_cache_remove(cache LRUCache, key size_t) -> LRUCache:
    size_t slot = _lru_find_slot(cache, key)
    if slot == (size_t)0x7FFFFFFF:
        return cache
    if cache.keys[slot] != key:
...
public def lru_cache_size(cache LRUCache) -> size_t:
    return cache.size

public def lru_cache_capacity(cache LRUCache) -> size_t:
    return cache.capacity

public def lru_cache_is_empty(cache LRUCache) -> int:
    if cache.size == (size_t)0:
        return 1
    return 0

public def lru_cache_clear(cache LRUCache) -> LRUCache:
    cache.head = (size_t)0x7FFFFFFF
    cache.tail = (size_t)0x7FFFFFFF
    cache.size = (size_t)0
    size_t i = (size_t)0
    for i in range((int)cache.capacity):
        ...
        cache.values[i] = 0
        cache.prev[i] = (size_t)0x7FFFFFFF
        cache.next[i] = (size_t)0x7FFFFFFF
